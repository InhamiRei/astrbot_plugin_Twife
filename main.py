...
class WifePlugin(Star):
    ...
    def __init__(self, context: Context):
        ...
        try:
            ...
            "停止学习": self.work_study_handler.stop_study,
            "打工列表": self.work_study_handler.work_list,
            "出门打工": self.work_study_handler.go_work,
            "停止打工": self.work_study_handler.stop_work,
            ...
            "咕咕嘎嘎排行": self.scratch_ranking_handler.scratch_ranking,
            "咕咕嘎嘎排名": self.scratch_ranking_handler.scratch_ranking,  # 别名
    ...
    async def on_all_messages(self, event: AstrMessageEvent):
        try:
            ...
            for command, func in self.commands.items():
                ...
                match_condition = message_str == command or (command in ["确认老婆", "牛老婆", "查老婆", "老婆详情", "赠送礼物", "出售物品", "购买物品", "出门学习", "停止学习", "出门打工", "停止打工", "购买家具", "出售家具", "家具中心-图片", "前往地下城", "一键出售战利品", "购买服装", "换衣", "脱下", "查询物品", "世界boss", "攻击boss", "咕咕嘎嘎", "咕咕嘎嘎池", "咕咕嘎嘎排行", "咕咕嘎嘎排名", "出门旅行", "捐赠文物", "刷新boss", "全体赔偿"] and message_str.startswith(command))
    ...
    async def wife_menu(self, event: AstrMessageEvent):
        ...
        menu += "21. 停止学习 - 提前结束学习并获得按实际小时数向下取整的收益\n"
        menu += "22. 打工列表 - 查看所有可用的打工工作\n"
        menu += "23. 出门打工 序号 - 让老婆去指定工作打工\n"
        menu += "24. 停止打工 - 提前结束打工并获得按实际小时数向下取整的收益\n"
        menu += "25. 家具中心 - 查看家具商店，购买各种家具装饰房产\n"
        menu += "26. 家具中心-图片 - 查看精美的家具目录图片\n"
        menu += "27. 购买家具 家具名 - 购买指定家具装饰房产\n"
        menu += "28. 出售家具 家具名 - 出售不需要的家具换取金币\n"
        menu += "29. 老婆详情 - 查询老婆的妹抖值、撒娇值、傲娇值、黑化率、反差萌及装备信息\n"
        menu += "30. 地下城列表 - 查看可进入的地下城列表\n"
        menu += "31. 前往地下城 序号 - 进入指定地下城进行冒险战斗\n"
        menu += "32. 一键出售战利品 - 快速出售所有战利品（地下城+世界Boss奖励）\n"
        menu += "33. 抽老婆菜单 - 显示本菜单\n"
        menu += "34. 服装商店 - 购买精品服装（兔女郎、女仆、巫女、魔法少女、小恶魔套装）\n"
        menu += "35. 购买服装 服装名 - 购买指定服装\n"
        menu += "36. 换衣 服装名 - 为老婆穿上指定服装\n"
        menu += "37. 脱下 服装名/部位 - 脱下指定服装或部位的装备\n"
        menu += "38. 查询物品 物品名 - 查看物品详情和效果\n"
        menu += "39. 世界boss - 查看当前世界Boss状态和伤害排行榜\n"
        menu += "40. 攻击boss - 攻击世界Boss，造成伤害（消耗30健康值）\n"
        menu += "41. 咕咕嘎嘎 [数量] - 花费100金币试试运气，有机会获得咕咕嘎嘎池大奖（固定奖励+奖池分成），可批量（如：咕咕嘎嘎 10）\n"
        menu += "42. 咕咕嘎嘎池 - 查看当前咕咕嘎嘎池状态和奖励说明\n"
        menu += "43. 咕咕嘎嘎排行 - 查看全服咕咕嘎嘎排行榜，统计次数、投入、收益、净收益等数据\n"
        menu += "44. 旅行列表 - 查看全世界可以旅行的地点，包含中国、日本、缅甸等地\n"
        menu += "45. 出门旅行 序号 - 前往指定地点旅行，获得碎片、纪念品和历史文物\n"
        menu += "46. 博物馆列表 - 查看各国博物馆和玩家捐赠记录\n"
        menu += "47. 捐赠文物 文物名称 - 将历史文物捐赠给博物馆，获得丰厚奖励\n"
        menu += "48. 我的捐赠记录 - 查看个人的文物捐赠历史和成就\n"
        menu += "49. 刷新boss [Boss名称] - 【管理员专用】刷新世界Boss和排行榜（可指定可可萝、大芋头王或圆头耄耋）\n"
        menu += "50. 全体赔偿 金币数量 - 【管理员专用】给所有用户赔偿指定数量的金币\n"
        menu += "\n【系统特色】\n"
        menu += "🎮 完全重构的模块化架构\n"
        ...
        menu += "🎫 咕咕嘎嘎系统：咕咕嘎嘎池模式运气游戏，支持批量操作（咕咕嘎嘎 数量），每次100金币进入咕咕嘎嘎池，三等奖(10万+30%)、二等奖(30万+50%)、一等奖(50万+80%)等你来拿\n"
        ...
        try:
            ...
            if len(parts) > 1:
                ...
                if "可可萝" in boss_param or "kkr" in boss_param.lower():
                    ...
                    boss_name = "大芋头王"
                elif "圆头耄耋" in boss_param or "耄耋" in boss_param or "圆头" in boss_param or "hjm" in boss_param.lower() or "基米" in boss_param:
                    boss_name = "圆头耄耋"
                else:
                    yield event.plain_result("❌ 无效的Boss名称！支持：可可萝、大芋头王、圆头耄耋")
                    return
